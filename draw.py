...
class Window(object):
    def __init__(self, display):
        ...
        self.window = self.screen.root.create_window(
            ...
            event_mask=(
                X.ExposureMask | X.StructureNotifyMask | X.ButtonPressMask
        ...
        self.window.set_wm_normal_hints(
            flags=(Xutil.PPosition | Xutil.PSize
                   | Xutil.PMinSize),
            min_width=20,
            min_height=20
        )
    ...
    def loop(self):
        ...
        while 1:
            ...
            if e.type == X.ButtonPress and e.detail == 1 and current:
                current.finish(e)
                current = None
...
class Movement(object):
    ...
    def motion(self, ev):
        ...
        self.win.window.poly_line(
            self.win.gc, X.CoordModeOrigin, self.lines[firstline:]
        )
    ...
    def expose(self, ev):
        ...
        if ev.count == 0:
            ...
            if self.last:
                ...
                self.win.window.poly_line(self.win.gc, X.CoordModeOrigin, self.lines)
                self.last.draw()
...
class Rhomboid(object):
    ...
    def draw(self):
        ...
        self.win.window.poly_line(
            self.win.gc, X.CoordModePrevious,
            [(self.x, self.y - 5), (5, 5), (-5, 5), (-5, -5), (5, -5)]
        )
